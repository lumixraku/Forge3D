...
import { createModelThumbnail } from '../model-thumbnail'
...
const emit = defineEmits<{
  ...
  'update-output': [generatedAssets: NodeGeneratedAssets]
...
const uploadAssets = computed(() => props.data.uploadAssets || {})
const referenceAssetType = computed(() => uploadAssets.value.assetType === 'model' ? 'model' : 'image')
const referenceAssetUrl = computed(() => typeof uploadAssets.value.assetUrl === 'string' ? uploadAssets.value.assetUrl : runtimePreview.value)
const referenceThumbnailUrl = computed(() => typeof uploadAssets.value.thumbnailUrl === 'string' ? uploadAssets.value.thumbnailUrl : '')
const visibleReferenceThumbnailUrl = computed(() => imageAvailable(referenceThumbnailUrl.value) ? referenceThumbnailUrl.value : '')
// Decides which of the two states the node's single asset slot shows: the preview
// or the drop zone. Never both, so the node keeps the same height either way.
const hasReferenceAssetPreview = computed(() => Boolean(isReferenceAsset.value
  && (referenceAssetType.value === 'model' ? referenceAssetUrl.value : visibleRuntimePreview.value)))
...
const runStateTitle = computed(() => {
  ...
  return needsInput.value ? 'Waiting for input' : 'Ready to run'
})
const runStateDetail = computed(() => {
  ...
  if (runtimeStatus.value !== 'running' && needsInput.value) return needsInputReason.value
...
// An uploaded image is shown from uploadAssets.assetUrl, because the upload writes
// only that tree; `preview` covers the older canvases that still carry the same URL
// under the generated one.
const runtimePreview = computed(() => localAssetPreview.value || props.nodeRun?.output?.preview || (!isExecutableNode.value ? props.data.generatedAssets?.preview || (isReferenceAsset.value && referenceAssetType.value === 'image' ? uploadAssets.value.assetUrl : '') : ''))
...
const runtimePreviews = computed(() => props.nodeRun?.output?.previews || (!isExecutableNode.value ? props.data.generatedAssets?.previews : []) || [])
const runtimeViewPreviews = computed(() => props.nodeRun?.output?.viewPreviews || (!isExecutableNode.value ? props.data.generatedAssets?.viewPreviews : {}) || {})
...
const visibleRuntimeViewPreviews = computed(() => Object.fromEntries(
  Object.entries(runtimeViewPreviews.value).filter(([, image]) => imageAvailable(image)),
))
...
function toggleApprove() {
  const next = !props.data.generatedAssets?.approved
...
function updateOutput(key: string, value: unknown) {
  emit('update-output', { ...props.data.generatedAssets, [key]: value })
}
...
function imageAvailable(source: unknown): source is string {
  return typeof source === 'string' && source.length > 0 && !failedImageSources.value.has(source)
}

function markImageFailed(source: unknown) {
  if (typeof source !== 'string') return
  failedImageSources.value = new Set(failedImageSources.value).add(source)
}
...
// Drops the uploaded asset so the node returns to its empty state. Also clears
// `preview` on the generated tree, because older canvases keep the uploaded URL
// there and the slot would otherwise keep showing it.
function clearAsset() {
  assetUploadError.value = ''
  const { assetType, assetUrl, modelUrl, reference, thumbnailUrl, ...rest } = uploadAssets.value
  emit('update-uploads', rest)
  if (props.data.generatedAssets?.preview) {
    const { preview, ...remaining } = props.data.generatedAssets
    emit('update-output', remaining)
  }
}
...
async function uploadAsset(file: File) {
  ...
  try {
    ...
    let thumbnailUrl = ''
    if (isModel && ['glb', 'gltf'].includes(extension)) {
      try {
        const thumbnail = await createModelThumbnail(file)
        const thumbnailResponse = await fetch('/api/assets', {
          method: 'POST',
          headers: { 'content-type': 'image/png', 'x-file-name': `${file.name}.thumbnail.png` },
          body: thumbnail,
        })
        const thumbnailResult = await thumbnailResponse.json().catch(() => null)
        if (thumbnailResponse.ok && typeof thumbnailResult?.url === 'string') thumbnailUrl = thumbnailResult.url
      } catch {
        // The asset is usable even if this optional, one-time thumbnail render fails.
      }
    }
    localAssetPreview.value = ''
    // An upload writes uploadAssets, never config and never generatedAssets: the
    // asset is input the user gave, so a copied node keeps it, while what a run
    // produced is dropped. That is why the URL goes to assetUrl/modelUrl and not
    // to `preview`, which belongs to the generated tree.
    emit('update-uploads', { ...uploadAssets.value, reference: file.name, assetType: isModel ? 'model' : 'image', assetUrl: result.url, ...(isModel ? { modelUrl: result.url, ...(thumbnailUrl ? { thumbnailUrl } : {}) } : {}) })
...
<template>
  <article class="forge3d-canvas-node forge:group forge:relative forge:w-[260px] forge:rounded-[10px] forge:border forge:border-[var(--node-accent)] forge:bg-[color-mix(in_srgb,var(--bg-input)_97%,transparent)] forge:px-[14px] forge:pb-[11px] forge:pt-[13px] forge:shadow-lg forge:transition-[border-color,box-shadow,filter,opacity] forge:hover:border-[color-mix(in_srgb,var(--node-accent)_72%,var(--line-strong))] forge:[&.forge3d-selected]:shadow-[inset_0_0_0_1px_var(--node-accent),var(--shadow-lg)] forge:[&.forge3d-is-running]:shadow-[inset_0_0_0_1px_color-mix(in_srgb,var(--node-accent)_45%,transparent),var(--shadow-lg)] forge:[&.forge3d-is-executing]:border-transparent forge:[&.forge3d-is-executing]:shadow-[inset_0_0_0_1px_color-mix(in_srgb,var(--node-accent)_18%,transparent),0_0_12px_color-mix(in_srgb,var(--node-accent)_18%,transparent),var(--shadow-lg)] forge:[&.forge3d-is-failed]:border-status-failed forge:[&.forge3d-is-failed]:shadow-[inset_0_0_0_1px_color-mix(in_srgb,var(--status-failed)_30%,transparent),var(--shadow-lg),0_0_22px_color-mix(in_srgb,var(--status-failed)_14%,transparent)]" :class="[`forge3d-tone-${data.tone}`, `forge3d-is-${runtimeStatus}`, { 'forge3d-selected': selected, 'forge3d-is-executing': isExecuting, 'forge3d-connection-invalid': connectionInvalid }]">
    ...
    <div class="forge:absolute forge:bottom-[calc(100%+7px)] forge:left-0 forge:z-[7] forge:inline-flex forge:h-[34px] forge:w-max forge:max-w-full forge:items-center forge:gap-2 forge:rounded-lg forge:border forge:border-[color-mix(in_srgb,var(--node-accent)_48%,var(--line-strong))] forge:bg-[color-mix(in_srgb,var(--node-accent)_16%,var(--bg-input))] forge:px-2 forge:py-[5px] forge:shadow-sm" :class="{ 'forge:outline forge:outline-1 forge:outline-[var(--node-accent)]': selected }">
    ...
    <div v-else-if="data.canvasType === 'generate-multiview-images' && Object.keys(visibleRuntimeViewPreviews).length" class="forge3d-node-output forge:relative forge:mb-[11px] forge:grid forge:aspect-[4/3] forge:w-full forge:grid-cols-2 forge:gap-0.5 forge:overflow-hidden forge:rounded-lg forge:border forge:border-line-subtle forge:bg-[radial-gradient(circle_at_50%_45%,#edf1ed,#dfe5e0_72%)] forge:p-0.5 forge:text-left forge:dark:bg-[radial-gradient(circle_at_50%_45%,#30352f,#111412_72%)]" aria-label="Generated multi-view images">
      <button v-for="view in viewPorts.filter((view) => visibleRuntimeViewPreviews[view])" :key="view" type="button" class="nodrag nopan forge:relative forge:min-h-0 forge:min-w-0 forge:overflow-hidden forge:rounded-[3px] forge:border-0 forge:bg-transparent forge:p-0 forge:transition-transform forge:focus-visible:outline forge:focus-visible:outline-2 forge:focus-visible:-outline-offset-2 forge:focus-visible:outline-[var(--node-accent)] forge:[&_img]:block forge:[&_img]:size-full forge:[&_img]:object-cover forge:[&_img]:transition-[filter] forge:hover:[&_img]:brightness-108" :aria-label="`Preview ${view} view`" @click.stop="emit('preview-image', { src: visibleRuntimeViewPreviews[view], alt: `${view} view` })">
        <img :src="visibleRuntimeViewPreviews[view]" :alt="`${view} view`" @error="markImageFailed(visibleRuntimeViewPreviews[view])" />
      </button>
      <div v-if="isExecuting" class="forge3d-node-output-loading forge:absolute forge:inset-0 forge:z-[5] forge:grid forge:place-content-center forge:justify-items-center forge:gap-[9px] forge:bg-[color-mix(in_srgb,var(--bg-input)_76%,transparent)] forge:text-center forge:text-text-primary forge:backdrop-blur-[3px]" role="status"><span class="forge3d-node-run-indicator" /><strong>{{ runtimeStatus === 'queued' ? 'Queued' : runtimeStatus === 'cancelling' ? 'Stopping' : 'Generating' }}</strong><div v-if="runtimeStatus === 'running'" class="forge3d-node-progress" :class="{ 'forge3d-indeterminate': runProgress === null }" :style="progressStyle"><span /><b>{{ runProgress === null ? 'Working' : `${runProgress}%` }}</b></div></div>
    </div>
    <div v-else-if="hasReferenceAssetPreview" class="forge3d-node-output forge:relative forge:mb-[11px] forge:aspect-[4/3] forge:w-full forge:overflow-hidden forge:rounded-lg forge:border forge:border-line-subtle forge:transition-[border-color,box-shadow] forge:hover:border-[var(--node-accent)]" :class="referenceAssetType === 'model' ? 'forge:bg-[radial-gradient(circle_at_50%_45%,#30352f,#111412_72%)]' : 'forge:bg-[radial-gradient(circle_at_50%_45%,#edf1ed,#dfe5e0_72%)] forge:dark:bg-[radial-gradient(circle_at_50%_45%,#30352f,#111412_72%)]'">
      <button type="button" class="nodrag nopan forge:grid forge:size-full forge:place-content-center forge:justify-items-center forge:border-0 forge:bg-transparent forge:p-4 forge:text-center forge:text-text-primary forge:focus-visible:outline forge:focus-visible:outline-2 forge:focus-visible:-outline-offset-2 forge:focus-visible:outline-[var(--node-accent)] forge:[&>img]:absolute forge:[&>img]:inset-0 forge:[&>img]:size-full forge:[&>img]:object-cover" :aria-label="referenceAssetType === 'model' ? `Open ${data.label} in Model Editor` : `Preview ${data.label} image`" @click.stop="referenceAssetType === 'model' ? emit('open-model-editor') : emit('preview-image', { src: visibleRuntimePreview, alt: `${data.label} result` })">
        <template v-if="referenceAssetType === 'model'">
          <img v-if="visibleReferenceThumbnailUrl" :src="visibleReferenceThumbnailUrl" :alt="`${data.label} thumbnail`" @error="markImageFailed(visibleReferenceThumbnailUrl)" />
          <svg v-else class="forge:mb-2 forge:size-11 forge:text-[var(--node-accent)] forge:drop-shadow-[0_0_14px_color-mix(in_srgb,var(--node-accent)_50%,transparent)]" viewBox="0 0 48 48" fill="none" aria-hidden="true"><path d="m24 5 15 8.5v17L24 39l-15-8.5v-17L24 5Z" stroke="currentColor" stroke-width="1.5"/><path d="m9 13.5 15 8.5 15-8.5M24 22v17" stroke="currentColor" stroke-width="1.5"/></svg>
          <strong class="forge:relative forge:z-[1] forge:max-w-full forge:overflow-hidden forge:text-ellipsis forge:whitespace-nowrap forge:font-mono forge:text-[9px] forge:font-semibold forge:uppercase" :class="{ 'forge:rounded forge:bg-black/60 forge:px-1.5 forge:py-1': visibleReferenceThumbnailUrl }">{{ uploadAssets.reference }}</strong>
          <small class="forge:relative forge:z-[1] forge:mt-1 forge:font-mono forge:text-[8px] forge:text-text-muted" :class="{ 'forge:rounded forge:bg-black/60 forge:px-1.5 forge:py-1 forge:text-white': visibleReferenceThumbnailUrl }">3D asset · click to preview</small>
        </template>
        <img v-else :src="visibleRuntimePreview" :alt="`${data.label} result`" @error="markImageFailed(visibleRuntimePreview)" />
      </button>
      <button type="button" class="nodrag nopan forge:absolute forge:right-[7px] forge:top-[7px] forge:z-[4] forge:grid forge:size-[18px] forge:place-items-center forge:rounded forge:border forge:border-white/15 forge:bg-[rgba(12,15,13,.76)] forge:p-0 forge:text-[11px] forge:leading-none forge:text-[#dce2dd] forge:backdrop-blur-[5px] forge:transition-[color,border-color] forge:hover:border-status-failed forge:hover:text-status-failed forge:focus-visible:outline forge:focus-visible:outline-2 forge:focus-visible:outline-offset-1 forge:focus-visible:outline-[var(--node-accent)]" aria-label="Remove uploaded asset" title="Remove uploaded asset" @click.stop="clearAsset" @pointerdown.stop>×</button>
      <span v-if="referenceAssetType !== 'model'" class="forge:pointer-events-none forge:absolute forge:bottom-[7px] forge:right-[7px] forge:z-[3] forge:rounded forge:border forge:border-white/15 forge:bg-[rgba(12,15,13,.76)] forge:px-1.5 forge:py-1 forge:font-mono forge:text-[7px] forge:font-medium forge:uppercase forge:text-[#dce2dd] forge:backdrop-blur-[5px]">Input asset</span>
      <span v-if="assetUploading" class="forge3d-node-output-loading forge:absolute forge:inset-0 forge:z-[5] forge:grid forge:place-content-center forge:justify-items-center forge:gap-[9px] forge:bg-[color-mix(in_srgb,var(--bg-input)_76%,transparent)] forge:text-center forge:text-text-primary forge:backdrop-blur-[3px]" role="status"><span class="forge3d-node-run-indicator" /><strong>Uploading asset</strong></span>
    </div>
    <button v-else-if="['generated-image', 'generate-model', 'smart-mesh', 'multiview-to-3d', 'text-to-3d', 'retopology', 'texture', 'rigging', 'segments', 'model-preview'].includes(data.canvasType) && visibleRuntimePreview" type="button" class="forge3d-node-output nodrag nopan forge:relative forge:mb-[11px] forge:block forge:aspect-[4/3] forge:w-full forge:overflow-hidden forge:rounded-lg forge:border forge:border-line-subtle forge:bg-[radial-gradient(circle_at_50%_45%,#edf1ed,#dfe5e0_72%)] forge:p-0 forge:text-left forge:transition-[border-color,box-shadow] forge:hover:border-[var(--node-accent)] forge:focus-visible:outline forge:focus-visible:outline-2 forge:focus-visible:outline-offset-2 forge:focus-visible:outline-[var(--node-accent)] forge:dark:bg-[radial-gradient(circle_at_50%_45%,#30352f,#111412_72%)] forge:[&>img]:size-full forge:[&>img]:object-cover forge:[&.forge3d-model-output>img]:relative forge:[&.forge3d-model-output>img]:z-[1] forge:[&.forge3d-model-output>img]:object-contain forge:[&.forge3d-model-output>img]:drop-shadow-[0_12px_12px_rgba(0,0,0,.45)] forge:[&.forge3d-model-output>img]:transition-[filter] forge:[&.forge3d-model-output:hover>img]:drop-shadow-[0_16px_16px_rgba(0,0,0,.5)]" :class="{ 'forge3d-model-output': data.canvasType !== 'generated-image' }" :aria-label="data.canvasType === 'generated-image' ? `Preview ${data.label} image` : `Open ${data.label} in Model Editor`" @click.stop="data.canvasType === 'generated-image' ? emit('preview-image', { src: visibleRuntimePreview, alt: `${data.label} result` }) : emit('open-model-editor')">
      <img :src="visibleRuntimePreview" :alt="`${data.label} result`" @error="markImageFailed(visibleRuntimePreview)" />
      <div v-if="data.canvasType !== 'generated-image'" class="forge:absolute forge:inset-[16px_30px_25px] forge:z-0 forge:rotate-[-12deg] forge:rounded-[50%] forge:border forge:border-[color-mix(in_srgb,var(--node-accent)_24%,transparent)] forge:[&_span]:absolute forge:[&_span]:size-1 forge:[&_span]:rounded-full forge:[&_span]:bg-[var(--node-accent)] forge:[&_span]:shadow-[0_0_8px_var(--node-accent)] forge:[&_span:nth-child(1)]:left-[21px] forge:[&_span:nth-child(1)]:top-2 forge:[&_span:nth-child(2)]:bottom-7 forge:[&_span:nth-child(2)]:right-0.5 forge:[&_span:nth-child(3)]:bottom-[-2px] forge:[&_span:nth-child(3)]:left-[44%]"><span /><span /><span /></div>
      <span class="forge:pointer-events-none forge:absolute forge:bottom-[7px] forge:right-[7px] forge:z-[3] forge:rounded forge:border forge:border-white/15 forge:bg-[rgba(12,15,13,.76)] forge:px-1.5 forge:py-1 forge:font-mono forge:text-[7px] forge:font-medium forge:uppercase forge:text-[#dce2dd] forge:backdrop-blur-[5px]">{{ data.canvasType === 'generated-image' ? 'Generated view' : data.canvasType === 'retopology' ? `${Number(data.config.faceLimit).toLocaleString()} faces` : data.canvasType === 'texture' ? `${data.config.textureQuality}` : data.canvasType === 'rigging' ? 'Rigged' : data.canvasType === 'segments' ? `Segments · ${data.config.detailLevel}` : data.canvasType === 'smart-mesh' ? 'Smart mesh' : '3D result' }}</span>
