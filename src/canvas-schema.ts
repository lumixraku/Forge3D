...
export interface NodePortSpec {
  ...
  required?: boolean
  /**
   * Named set of ports where at least one must be satisfied. `required` is
   * per-port AND-semantics; this is OR-semantics across the group, for a node
   * that accepts any one of several sources.
   */
...
/** Marks ports as alternatives: at least one of the group must be satisfied. */
const anyOf = (group: string, ports: NodePorts): NodePorts => Object.fromEntries(Object.entries(ports).map(([key, port]) => [key, { ...port, requiredGroup: group }]))
