...
- Aligned the run-state placeholder card to 146px, matching every preview card in the same `v-if` chain. It was 148px, so a finishing run shrank the node by 2px, which cleared `fitFrameNodes`' 0.5px threshold and drove the refit above.
- Removed `resizingFrameId`, `frameFitShouldSave`, and `queueFrameFit`'s `persist` option, which the change above left unread.
- Fixed the first attempt at the above, which went too far and stopped a node dragged into a section from resizing it at all. Removing the `dimensions` branch also removed the only thing that reacted to Vue Flow stretching a frame's own rendered box, so `onNodeDragStop`'s bare `fitFrames()` wrote a fitted size into state while the rendered frame kept the size Vue Flow had given it. Drag stop now awaits `fitFramesAfterRender()`, which fits after the reparent renders and re-measures via `updateNodeInternals`.
- Retired the `manualSize` flag, which made manual and automatic sizing mutually exclusive modes. It latched permanently on the first hand-resize and, worse, `buildDrawnFrame` set it at birth, so a section drawn on the canvas never auto-fit to nodes entering or leaving it. Sections now always fit their children; a hand-resize is just another size. `toDomainCanvas` strips the flag from canvases saved before this change, and `onFrameResizeStart` plus the `resize-start` emit it existed to serve are gone.
- Split `fitView` out of `autoLayout`, which had been recentering the viewport as a side effect of laying out. The toolbar's Auto layout now calls `autoLayout().then(fitCanvasView)`, so callers that only want layout get only layout.
- Collapsed `scheduleLayoutSave` into `scheduleSave`. The canvas had two save paths differing only in debounce (700ms for edits, 10s for geometry), each with its own timer, both calling `markWorkflowDirty()` and `saveCanvas()`. Saving is a property of the canvas being dirty, not of which interaction dirtied it. One path now, and the `hasUnsavedCanvasChanges()` diff that used to guard only the layout path guards every save — that diff, not the delay, is what stops a fit triggered by applying a collaborator's canvas from broadcasting it back and trading revisions forever. Geometry now saves ~700ms after the interaction settles instead of up to 10s later.
- Added `src/canvas-frames.test.js`, the first coverage for `useCanvasFrames`: dragging a node into a section adopts it and grows the section (asserting `style.height` and re-measurement, not just `width`/`height`), dragging out shrinks it, and a node re-measuring mid-run leaves its section untouched. Confirmed the drag-in test fails when the `fitFrames()` regression is reintroduced.
...
- Verification for the frame-sizing changes: `pnpm test` passed 269 tests, `pnpm typecheck`, `pnpm build`, and `git diff --check` passed. The build retains the existing large-chunk warning. The three new `useCanvasFrames` tests cover drag in, drag out, and the mid-run re-measure; the drag-in test was confirmed to fail against the reintroduced bug. In the browser, a reload showed the previously stuck section refit to its children and its state and rendered box agree (needed height equals actual, child inside the frame), and the retired `manualSize` no longer appears in the loaded graph. Not verified in a browser: a genuine mouse drag of a node into a section. Vue Flow's drag needs trusted input events, and synthetic ones did not engage it; one such attempt threw midway and left a node reparented without a fit, which produced a misleading intermediate reading before the page was reloaded.
- Verification for the save merge: `pnpm test` passed 269 tests, `pnpm typecheck`, `pnpm build`, and `git diff --check` passed. Checked every `scheduleSave` caller, since the merged path now gates on `hasUnsavedCanvasChanges()` and the edit path previously did not: all of them mutate nodes or edges, which that diff compares, and canvas rename goes through its own `PATCH` rather than this scheduler. Not verified in a browser.
- Auto layout works; an earlier note here claiming it crashed was wrong. The `TypeError: Cannot read properties of null (reading 'emitsOptions')` came from invoking `autoLayout()` directly out of a devtools evaluation context, which renders Vue components whose vnodes have no instance in that realm — the stack ended in `pptr:evaluateHandle`, which should have been the tell. Reproducing it on unmodified `HEAD` only reproduced the bad harness. Clicking the real toolbar button raises no errors and relayouts correctly (verified on the standard-workflow canvas: `reference-image` moved x 1734 → -74, `frame-main` y -19 → 510).
- Remaining issues: The removed dimensions branch also covered genuine height changes from user interaction (expanding Advanced parameters, uploading an asset). A section containing such a node no longer refits until the next position change or drag, so it can sit visually loose around its children until then. Worth a real mouse-drag pass in the browser to confirm the drag path end to end.
...
- Fixed adding a node never moving the canvas to it. `addNode` called `fitView` inside a bare `nextTick()`, but `fitView` skips any node whose measured `dimensions` are still zero, and a freshly rendered node is measured by a ResizeObserver that had not fired yet — so it found nothing to fit and the viewport stayed put, leaving the new node wherever it happened to land, often off-screen. The existing `focusNodes` helper had the same latent problem, waiting a fixed single frame. Both now poll `findNode(id).dimensions` until the node is really measured (capped at 30 frames) before fitting, so a new node lands centred at up to 100% zoom — matching what the `frame` branch already got right via `focusNode`.
- Verified that fix in Chrome: before it, the transformation pane transform was byte-identical before and after adding a node, which sat partly outside the pane at 0.34 zoom; after it, the viewport animates and the node ends exactly centred (`dx: 0, dy: 0`), fully visible, at zoom 1. The connected path via a node's own `+` button also centres the new node and creates its edge. Zoom is deliberately capped at `maxZoom: 1`, so a small node does not fill the pane; going closer would blur text and drop surrounding context. Nodes created while probing were deleted, restoring the local dev canvas.
